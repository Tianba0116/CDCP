...
class Attention(nn.Module):
    ...
    def forward(self, x, mask_modality, mask=None):
        ...
        if mask is not None:
            mask = mask.bool()
            mask_dict = {'a': mask[:, :seq_len], 't': mask[:, seq_len:2*seq_len], 'v': mask[:, 2*seq_len:3*seq_len]}
...
class Block_softmoe(nn.Module):
    def __init__(self, dim, num_heads=8, attn_drop=0, proj_drop=0, mlp_ratio=1):
        super().__init__()
    ...
    def forward(self, x, cross_modality, mask_modality, mask=None):
        if cross_modality == 'a': return self.Transformer_a(x, mask_modality, mask)
        if cross_modality == 't': return self.Transformer_t(x, mask_modality, mask)
        if cross_modality == 'v': return self.Transformer_v(x, mask_modality, mask)
        return x


class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4.0, drop=0.0, attn_drop=0.0, depth=4):
        super().__init__()
        self.blocks = nn.ModuleList([
            Block_softmoe(dim, num_heads, attn_drop, drop, mlp_ratio) for _ in range(depth)
        ])

    def forward(self, x, mask=None, modality=None):
        x_cross_a, x_cross_t, x_cross_v = x.clone(), x.clone(), x.clone()
...
class FrequencySplitter(nn.Module):
    ...
    def forward(self, x):
        ...
        cutoff_idx = int(freq_len * self.cutoff_ratio)
        cutoff_idx = max(1, cutoff_idx)

        mask_low = torch.zeros_like(x_freq)
        mask_low[:, :cutoff_idx, :] = 1.0

        mask_high = torch.ones_like(x_freq)
        mask_high[:, cutoff_idx:, :] = 1.0

        x_freq_low = x_freq * mask_low
        x_freq_high = x_freq * mask_high

        x_low = torch.fft.irfft(x_freq_low, n=S, dim=1)
        x_high = torch.fft.irfft(x_freq_high, n=S, dim=1)
...
class MoMKE(nn.Module):
    def __init__(self, args, adim, tdim, vdim, D_e, n_classes=4, depth=4, num_heads=8, mlp_ratio=1, drop_rate=0, attn_drop_rate=0, no_cuda=False):
        ...
        self.block = Block(dim=D_e, num_heads=num_heads, mlp_ratio=mlp_ratio, drop=drop_rate, attn_drop=attn_drop_rate, depth=depth)

        router_input_dim = 5 * D_e
        self.router_a = Mlp(in_features=router_input_dim, hidden_features=int(D_e * mlp_ratio), out_features=3, drop=drop_rate)
        self.router_t = Mlp(in_features=router_input_dim, hidden_features=int(D_e * mlp_ratio), out_features=3, drop=drop_rate)
        self.router_v = Mlp(in_features=router_input_dim, hidden_features=int(D_e * mlp_ratio), out_features=3, drop=drop_rate)
    ...
    def forward(self, inputfeats, umask, spk_embeddings):
        ...
        if umask is None:
            umask = torch.ones(B, seq_len).to(inputfeats.device)
        attn_mask = torch.cat([umask, umask, umask], dim=1)

        mask_float = umask.unsqueeze(-1)

        proj_a = self.pos_emb(self.ln_a(self.a_in_proj(audio)), spk_embeddings)
        proj_t = self.pos_emb(self.ln_t(self.t_in_proj(text)), spk_embeddings)
        proj_v = self.pos_emb(self.ln_v(self.v_in_proj(video)), spk_embeddings)

        proj_a = proj_a * mask_float
        proj_t = proj_t * mask_float
        proj_v = proj_v * mask_float

        low_a, high_a = self.freq_splitter(proj_a)
        low_t, high_t = self.freq_splitter(proj_t)
        low_v, high_v = self.freq_splitter(proj_v)

        consensus_feat = self.consensus_fusion(low_t, low_v, low_a)

        consensus_feat = consensus_feat * mask_float

        diff_a = high_a + (low_a - consensus_feat)
        diff_t = high_t + (low_t - consensus_feat)
        diff_v = high_v + (low_v - consensus_feat)

        diff_a = diff_a * mask_float
        diff_t = diff_t * mask_float
        diff_v = diff_v * mask_float
        ...
        if self.training:
            ...
            def masked_ortho_loss(feat_diff, feat_common, m):
                ...
                if m.sum() > 0:
                    ...
                    return torch.tensor(0.0, device=feat_diff.device)
        ...
        global_context = torch.cat([consensus_feat, modality_variance, diff_a, diff_t, diff_v], dim=-1)
        ...
        return hidden, None, None, ortho_loss
